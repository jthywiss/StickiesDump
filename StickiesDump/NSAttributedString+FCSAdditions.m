								 ...
								 withTitle:(NSString *)htmlTitle
{
	...
	BOOL underline = NO, italic = NO, bold = NO, tt = NO;
...
//			[css setString:@""];
            if (italic)
                temp = [NSString stringWithFormat:@"<i>%@</i>", temp];
			if (bold)
				temp = [NSString stringWithFormat:@"<b>%@</b>", temp];
			if (underline && !link)
                temp = [NSString stringWithFormat:@"<u>%@</u>", temp];
//			if (c)
